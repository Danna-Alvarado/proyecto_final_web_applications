    ...
    (err, result) => {
      ...
      db.query("INSERT INTO ventas (album_id, cantidad, total) VALUES (?, ?, ?)",
        ...
        (err) => {
          ...
          if (err) return res.send("Error al vender");
...
app.get("/buscar", verificarSesion, (req, res) => {
  ...
  const query = `SELECT * FROM albums WHERE tittle LIKE ?`;
...
if (err) {
console.error(err);
return res.send("Error al obtener albums");
}
...
if (err) {
console.error(err);
return res.send("Error al agregar album");
}
...
if (err) {
console.error(err);
return res.send("Error al actualizar album");
}
...
if (err) {
console.error(err);
return res.send("Error al obtener ventas del día");
}
...
if (err) {
console.error(err);
return res.send("Error al obtener historial");
}
...
if (err) {
console.error(err);
return res.send("Error al obtener total del día");
}
